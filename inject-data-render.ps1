# Script pour injecter les donnees sur Render PRODUCTION
Write-Host "Injection des donnees VEG'N BIO sur Render PRODUCTION" -ForegroundColor Green
...
Write-Host "`nTest de connexion a Render PRODUCTION..." -ForegroundColor Yellow
...
Write-Host "`nAuthentification admin sur Render..." -ForegroundColor Yellow
$loginData = @{
...
Write-Host "`n1. Creation des utilisateurs sur Render..." -ForegroundColor Yellow
...
foreach ($user in $users) {
    ...
    if ($userResponse) {
        ...
        Write-Host "⚠️ Utilisateur $($user.role) peut-etre deja existant sur Render: $($user.email)" -ForegroundColor Yellow
    }
}

# 2. REMPLISSAGE DES MENUS ET PLATS
Write-Host "`n2. Creation des menus et plats sur Render..." -ForegroundColor Yellow
...
if ($restaurants) {
    foreach ($restaurant in $restaurants) {
        ...
        if ($menuDejeunerResponse) {
            Write-Host "  ✅ Menu Dejeuner cree sur Render" -ForegroundColor Green
...
Write-Host "`n3. Creation des evenements sur Render..." -ForegroundColor Yellow
...
Write-Host "`n4. Creation des reservations sur Render..." -ForegroundColor Yellow
...
Write-Host "`n5. Creation des rapports sur Render..." -ForegroundColor Yellow
...
Write-Host "`n6. Verification finale des donnees sur Render..." -ForegroundColor Yellow
...
Write-Host "`n🎉 INJECTION RENDER PRODUCTION TERMINEE !" -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Green

Write-Host "`n📊 Donnees injectees sur Render:" -ForegroundColor Cyan
...
Write-Host "`n🌐 Acces Render PRODUCTION:" -ForegroundColor Cyan
...
Write-Host "`n🚀 VOTRE APPLICATION VEG'N BIO EST COMPLETE SUR RENDER !" -ForegroundColor Green
