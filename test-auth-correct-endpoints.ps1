#!/usr/bin/env pwsh
# Test du flux d'authentification avec les bons endpoints
# VegN-Bio Backend - Endpoints corrects
...
Write-Host "🔐 Test du Flux d'Authentification - Endpoints Corrects" -ForegroundColor Blue
...
Write-Host "0️⃣ Test de connectivité de base..." -ForegroundColor Yellow
try {
...
# Test 2: Enregistrement d'utilisateur avec le bon endpoint
Write-Host ""
...
if ($Verbose) {
    ...
    Write-Host "   Endpoint: $BackendUrl/api/v1/auth/register" -ForegroundColor Gray
}
...
try {
    ...
    if ($Verbose) {
        Write-Host "   Endpoint testé: $BackendUrl/api/v1/auth/register" -ForegroundColor Gray
        Write-Host "   Status Code: $($_.Exception.Response.StatusCode)" -ForegroundColor Gray
    }
}

# Test 3: Connexion utilisateur avec le bon endpoint
Write-Host ""
...
if ($Verbose) {
    ...
    Write-Host "   Endpoint: $BackendUrl/api/v1/auth/login" -ForegroundColor Gray
}
...
try {
    ...
    if ($Verbose) {
        Write-Host "   Endpoint testé: $BackendUrl/api/v1/auth/login" -ForegroundColor Gray
        Write-Host "   Status Code: $($_.Exception.Response.StatusCode)" -ForegroundColor Gray
    }
}
...
if ($token) {
    ...
    try {
        ...
        if ($Verbose) {
            Write-Host "   Endpoint: $BackendUrl/api/v1/auth/me" -ForegroundColor Gray
            Write-Host "   Headers: Authorization: Bearer $($token.Substring(0, [Math]::Min(20, $token.Length)))..." -ForegroundColor Gray
        }
        ...
        if ($Verbose) {
            Write-Host "   Endpoint testé: $BackendUrl/api/v1/auth/me" -ForegroundColor Gray
            Write-Host "   Status Code: $($_.Exception.Response.StatusCode)" -ForegroundColor Gray
        }
    }
} else {
...
# Test 5: Test d'accès aux endpoints protégés
if ($token) {
    Write-Host ""
    ...
    try {
        ...
        if ($Verbose) {
            Write-Host "   Test d'accès aux restaurants..." -ForegroundColor Gray
        }
...
# Test 6: Test de sécurité (accès sans token)
Write-Host ""
...
# Test 7: Test de redirection (simulation)
Write-Host ""
...
if ($authSuccess) {
    ...
    # Test de redirection réelle
    Write-Host ""
    Write-Host "   🎯 Test de redirection réelle..." -ForegroundColor Yellow
    try {
        $headers = @{
            "Authorization" = "Bearer $token"
        }
        $redirectResponse = Invoke-RestMethod -Uri "$BackendUrl/api/v1/restaurants" -Method GET -Headers $headers -TimeoutSec 30 -ErrorAction Stop
        Write-Host "   ✅ Redirection réussie - Accès à l'accueil confirmé!" -ForegroundColor Green
        Write-Host "   📊 Données de l'accueil récupérées: $($redirectResponse.Count) éléments" -ForegroundColor Cyan
    } catch {
        Write-Host "   ❌ Échec de la redirection: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
...
# Test 8: Test de performance du flux d'authentification
Write-Host ""
...
Write-Host ""
Write-Host "🔗 URLs de test (endpoints corrects):" -ForegroundColor Blue
Write-Host "- Backend: $BackendUrl"
